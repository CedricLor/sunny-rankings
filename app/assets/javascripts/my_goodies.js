$(window).load( function() {
  // 1. SMOOTH SCROLLING
  // The css selector selects each <a> element which href attribute starts with #
  // but not with href attribute that contains only the #
  $('a[href*=#]:not([href=#])').click(function() {
    // the first if controls the pathname (i.e. url without domain name) and after having dropped the initial
    ...
    if (location.pathname.replace(/^\//,'') == this.pathname.replace(/^\//,'') && location.hostname == this.hostname) {
      ...
      // checks that the selection has selected an object; if not, tries to select an element
